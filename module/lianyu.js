...
*创建时间：2024-2-18
*/
//class com.modules.boss.lianyu.BossLianyuView extends ui.mobile.boss.lianyu.BossLianyuViewUI
...
*创建时间：2024-2-19
*/
//class com.modules.boss.lianyu.LianyuCenter
var LianyuCenter=(function(){
	function LianyuCenter(){}
	__class(LianyuCenter,'com.modules.boss.lianyu.LianyuCenter');
	LianyuCenter.update=function(cmd){
		if(!SetupCenter.isInit){
			Laya.workerTimer.once(2000,LianyuCenter,com.modules.boss.lianyu.LianyuCenter.update,[cmd],false);
			return;
		};
		var old=LianyuCenter._dic[cmd.zoneModelId];
		if(!old){
			old=new LianyuData();
			LianyuCenter._dic[cmd.zoneModelId]=old;
			if(cmd.zoneModelId==200000){
				old.funId=102;
			}
		}
		old.update(cmd);
		EventMgr.dispatch("LianyuCenter.UPDATE");
		BossDataCenter.onSend();
	}

	LianyuCenter.getData=function(zoneId){
		return LianyuCenter._dic[zoneId];
	}

	LianyuCenter.isRed=function(zoneId){
		return LianyuCenter._dic[zoneId] ? (LianyuCenter._dic [zoneId]).red :false;
	}

	LianyuCenter.updateRed=function(zoneId){
		if(LianyuCenter._dic[zoneId]){
			LianyuCenter.getData(zoneId).updateRed();
		}
	}

	LianyuCenter.UPDATE_ZONE="LianyuCenter.UPDATE_ZONE";
	LianyuCenter.UPDATE="LianyuCenter.UPDATE";
	LianyuCenter.cur_layer=0;
	LianyuCenter.cur_boss=0;
	LianyuCenter._dic={};
	return LianyuCenter;
})()
...
*创建时间：2024-2-19
*/
//class com.modules.boss.lianyu.LianyuData
var LianyuData=(function(){
	function LianyuData(){
		this._dic={};
		this._red=false;
		this.funId=0;
	...
	__class(LianyuData,'com.modules.boss.lianyu.LianyuData');
	var __proto=LianyuData.prototype;
	__proto.update=function(cmd){
		this.zoneId=cmd.zoneModelId;
		var old;
		var info;
		for(var $each_info in cmd.boInfos){
			info=cmd.boInfos[$each_info];
			old=this._dic[info.boId];
			if(!old){
				old=new LianyuBoData();
				this._dic[info.boId]=old;
			}
			old.finishFlag=info.finishFlag;
			old.monsters=info.monsterInfos;
			old.canSweep=(cmd.sweepFlag & (1 << EnumZoneBoType.getBo(info.boId)-1))!=0;
			old.mapid=BossDataCenter.instance.updateLianyuHJ(info,this);
		}
		this.updateRed();
	}

	__proto.getLayer=function(value){
		return this._dic[value];
	}

	/**是否已通关*/
	__proto.isFinish=function(boid){
		var l_info=this.getLayer(boid);
		return l_info && (l_info.canSweep || l_info.finishFlag==1);
	}

	__proto.updateRed=function(){
		this._red=TaskModel.canGetBranchTask(69);
		if(!this._red){
			var beans=App.dataMgr.q_zone_boContainer.getListByType(10);
			for(var i=0;i < beans.length;i++){
				if(this.isTabRed(beans[i].q_id,false)){
					this._red=true;
					break ;
				}
			}
	...
	__proto.isTabRed=function(boid,isTask){
		(isTask===void 0)&& (isTask=true);
		var l_info=this.getLayer(boid);
		if(l_info){
			var tm_id=0;
			if(isTask){
				var task=TaskModel.getBranchTaskList(69)[0];
				if(task && task.taskState==2){
					tm_id=myparseInt(App.dataMgr.q_taskBranchContainer.getDataBean(task.taskID).q_task_des);
				}
			};
			var m_info;
			if(l_info.canSweep){
				var $each_m_info;
				for($each_m_info in l_info.monsters){
					m_info=l_info.monsters[$each_m_info];
					if(m_info.sweepCount > 0 || m_info.modelId==tm_id){
						return true;
					}
				}
			}
				...
				for($each_m_info in l_info.monsters){
					m_info=l_info.monsters[$each_m_info];
					if(m_info.modelId==tm_id || (m_info.receiveTimestamp <=stime && BossDataCenter.instance.isGuanzhu(l_info.mapid,m_info.modelId))){
						return true;
					}
				}
			}
	...
	__getset(0,__proto,'red',function(){
		return FunctionManager.isFunctionOpen(this.funId)? this._red :false;
	});
